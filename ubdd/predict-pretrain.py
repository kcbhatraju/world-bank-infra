...
def ubdd_plusplus(
    ...
    with tqdm(dataloader, total=len(dataloader)) as pbar:
        for batch in pbar:
            ...
            # Right after get_dino_output
            print("-" * 30)
            print(f"DINO Output Keys: {output.keys()}")
            print(f"DINO Scores: {output['scores']}")
            print(f"Number of Bounding Boxes: {len(output['boxes'])}")
            print("-" * 30)
            ...
            if save_annotations:
                ...
                # exit(0)
                # print(f"Saved {file_name}. Max pixel value in output: {color_mask.max()}")
...
def main():
    ...
    xbd_dataloader = DataLoader(xbd_dataset, batch_size=1, shuffle=False, num_workers=1)
